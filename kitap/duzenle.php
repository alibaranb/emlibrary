...
<!DOCTYPE html>
<head>
    <title>Kitap düzenle - Kütüphane takip sistemi</title>
...
$sorgu = $baglanti->query("SELECT * FROM kitap WHERE id =".(int)$_GET['id']); 
...
<form action="" method="post">
    ...
    <thead class="thead-dark">
        <tr>
        <th style="text-align:center" colspan="2">Kitap Düzenle</th>
        </tr>
    </thead>
    <tbody class="bg-white">
        <tr>
            <td>Kitap Adı :</td>
            <td><input type="text" name="kitap_ad" maxlength="100" autocomplete="off" class="form-control" value="<?php echo $sonuc['ad']; 
        ...
        <tr>
            ...
            <td><input type="text" name="kitap_yazar" maxlength="100" autocomplete="off" class="form-control" value="<?php echo $sonuc['yazar'];?>"></td>
        </tr>
...
if ($_POST) { // Post olup olmadığını kontrol ediyoruz.
    ...
    $kitap_ad = $_POST['kitap_ad']; // Post edilen değerleri değişkenlere aktarıyoruz
    $kitap_yazar = $_POST['kitap_yazar'];

    if ($kitap_ad<>"" && $kitap_yazar<>"") { // Veri alanlarının boş olmadığını kontrol ettiriyoruz.
        
        // Veri güncelleme sorgumuzu yazıyoruz.
        if ($baglanti->query("UPDATE kitap SET ad = '$kitap_ad', yazar = '$kitap_yazar' WHERE id =".$_GET['id'])) 
        {
        ...
        {
            echo "Kitap güncellenirken bir hata oluştu"; // id bulunamadıysa veya sorguda hata varsa hata yazdırıyoruz.
        }
    }
}
?>
